...
export default class MongoDB extends MongoClient {
    ...
    public findOne<T>(collection: string, filter: Partial<T>, db: string = process.env.DB_NAME): Promise<T> {
    ...
    public async findOrInsert<T>(collection: string, filter: Partial<T>, data: object, db: string = process.env.DB_NAME): Promise<T> {
